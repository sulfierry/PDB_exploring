...
def lennard_jones_potential(atom1, atom2, residue, r):
    ...
    # Este cálculo refere-se à combinação dos parâmetros de profundidade do poço de
    # energia epsilon para dois átomos diferentes quando se modela uma interação via potencial de
    # Lennard-Jones.  A combinação geométrica (média geométrica) é comum para este parâmetro.
    epsilon_combined = (epsilon1 * epsilon2) ** 0.5

    # Este cálculo refere-se à combinação dos parâmetros de distância de sigma para 
    # os mesmos dois átomos. sigma é geralmente interpretado como a distância em que o
    # potencial interatômico entre dois átomos neutros é zero.
...
# Realizar o parse do PDB tradicional para o PDB contendo os atomos formatados no estilo MMFF
